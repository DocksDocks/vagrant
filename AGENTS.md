...
This repository provisions a complete Debian development VM with XFCE desktop. The default (x86_64 — Windows, Linux, Intel Mac) target is **Debian 13 (Trixie)** on **VirtualBox**; on **macOS Apple Silicon (ARM64)** it targets **Debian 12 (Bookworm)** on **UTM** (VirtualBox has no ARM build). The `Vagrantfile` is thin (~300 lines) and delegates each concern to a numbered shell script under `scripts/`, with static configs under `assets/`. Scripts are fetched from `raw.githubusercontent.com/${SCRIPTS_REPO}/${SCRIPTS_REF}/scripts/` at provision time, or read from `/vagrant/assets/` if `VAGRANT_SCRIPTS_DIR` is set (local-dev mode). See `plans/0002-split-vagrantfile.md` for the split-Vagrantfile design and `plans/0009-multi-platform-arm64.md` for the multi-platform (Apple Silicon) decision.
...
- **Base box (Apple Silicon ARM64):** `utm/bookworm` (Debian 12 Bookworm, arm64), selected automatically when the host is a Mac ARM (`is_mac_arm` in the Vagrantfile) and overridable via `VAGRANT_ARM_BOX`. There is **no** Debian 13 Trixie arm64 box published for any *free* Vagrant provider (the only maintained Trixie aarch64 box is VMware-only), and no `bento/debian-13` aarch64 build exists. Bookworm ships PHP 8.2, so PHP 8.4 (a project requirement) comes from the **Sury repo** on the ARM path — see `10`/`20-*.sh`. Full rationale in `plans/0009-multi-platform-arm64.md`.
...
The `Vagrantfile` (~300 lines) handles host-side resource detection and the provider config (VirtualBox on x86_64, UTM on Apple Silicon — gated by `is_mac_arm`), then iterates `SCRIPTS = %w[...]` and registers one shell provisioner per script. Each provisioner either runs the local file (`VAGRANT_SCRIPTS_DIR=./scripts`) or fetches it from `raw.githubusercontent.com` at run time. On Apple Silicon the VirtualBox-specific scripts (`VBOX_ONLY_SCRIPTS`: `30`/`50`/`51`) are skipped — they would abort under `set -euo pipefail` with no VBox present.
...
2. **Provider config**: on x86_64, the **VirtualBox** block — VM resources, VMSVGA graphics, bidirectional clipboard, drag-and-drop, audio. On Apple Silicon, the **UTM** block (`config.vm.provider "utm"`) — `name`/`memory`/`cpus` + `directory_share_mode = "virtFS"` (9p mount of `/vagrant` for local-dev mode; the default mode fetches scripts from GitHub). Selected by the `if is_mac_arm` branch.
3. **Per-concern shell scripts** (`scripts/`):
   - `10-apt-repos.sh` — system update, timezone, grub-pc seed, external repos (Chrome **[amd64 only]**, Docker, gh, VS Code). On Bookworm/Bullseye it also adds the **Sury PHP repo** (via the `debsuryorg-archive-keyring.deb` package) so PHP 8.4 is installable on arm64; Trixie has 8.4 natively so Sury is skipped there.
   ...
   - `20-packages.sh` — batch `apt install`, **explicit `php8.4-*` packages** (pinned 8.4 on both arches: native on Trixie, Sury on Bookworm), Composer (with SHA-384 verification), docker group, vagrant password. Arch-conditional browser + guest tooling: amd64 gets `google-chrome-stable` (+ removes the duplicate `google-chrome.sources` deb822 source); arm64 gets `chromium` + `spice-vdagent` + `qemu-guest-agent` (UTM clipboard/resize/clean-shutdown, replacing the skipped VBox clipboard scripts).
   ...
   - `40-xfce-base.sh` — LightDM autologin, panel/dock layout, default browser (Chrome on amd64, Chromium on arm64), Chrome no-GPU policy (amd64 only — it's Chrome/VMSVGA-specific).
...
- **Apple Silicon (macOS ARM64) — provider/box/PHP:** VirtualBox has no ARM build, so the Vagrantfile auto-switches (`is_mac_arm`) to **UTM** (`vagrant_utm` plugin) + `utm/bookworm` (Debian 12 arm64). Prereqs on the host: `brew install --cask utm` and `vagrant plugin install vagrant_utm`. Why not `vagrant-qemu`: it's headless (`-display none`) — you'd lose the whole XFCE desktop. Why not Debian 13: no Trixie arm64 box exists for a free provider (the only one is VMware-only). PHP 8.4 (required) comes from the **Sury repo** since Bookworm ships 8.2 — see `10`/`20-*.sh`. The VBox scripts (`30`/`50`/`51`) are skipped; clipboard/resize come from `spice-vdagent` (install present) + UTM's SPICE channel — if clipboard doesn't work, confirm the VM's display is SPICE and `systemctl status spice-vdagentd` in the guest. Override the box with `VAGRANT_ARM_BOX` (e.g. a self-built Trixie arm64 image). Full rationale + alternatives table: `plans/0009-multi-platform-arm64.md`.
