...
3. **Per-concern shell scripts** (`scripts/`):
   ...
   - `66-vscode.sh` — VS Code user `settings.json` (first-provision seed) + extensions from `assets/vscode/extensions.txt` (seeded with the Night Owl theme). `code` is installed from the Microsoft apt repo (10/20).
