...
3. **Per-concern shell scripts** (`scripts/`):
   - `10-apt-repos.sh` — system update, timezone, grub-pc seed, external repos (Chrome, Docker, gh, VS Code).
   ...
   - `20-packages.sh` — batch `apt install`, Composer (with SHA-384 verification), docker group, vagrant password. Also removes the duplicate Chrome deb822 source (`google-chrome.sources`) the Chrome `.deb` drops, so apt stops warning that the repo is configured twice (our `.list` in `10` is the canonical one).
