...
**Appending to ~/.bashrc without grep-q guard**: each `vagrant provision` run duplicates the block. Four scripts currently append to bashrc — all use the grep-q pattern. Source: `scripts/80-git-ssh.sh:24-30`, `scripts/85-secrets-env.sh:50`, `scripts/60-tilix.sh:75`, `scripts/70-nodejs-claude.sh:60`.
