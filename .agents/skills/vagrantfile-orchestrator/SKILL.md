...
<constraint>
On **Apple Silicon (ARM64)** the box/provider are chosen by `is_mac_arm` — `ARM_BOX` (default `utm/ubuntu-24.04`, Ubuntu 24.04 LTS; `utm/bookworm` selectable via the env var) on the **UTM** provider (`vagrant_utm`). Do NOT change this to VirtualBox (no usable ARM build) or `vagrant-qemu` (headless — drops the XFCE desktop), and do NOT "fix" the box to Debian 13 (no free-provider Trixie arm64 box exists). Scripts detect Debian vs Ubuntu via `/etc/os-release` (`10`/`20` adapt Docker/PHP repos + browser; `41`/`45` and parts of `40` are Debian-only and self-skip on Ubuntu). The VirtualBox-only scripts `30`/`50`/`51` (`VBOX_ONLY_SCRIPTS`) MUST stay skipped on this path or provisioning aborts under `set -euo pipefail`. Source: `Vagrantfile`, `plans/0009-multi-platform-arm64.md`, `docs/macos-apple-silicon.md`.
</constraint>
